...
class AwsGlue(AwsBase):
    ...
    '''
    def create_rsf_array_table(self):
        
        

        

        partitionsArray = [{'averageRecordSize': '17',
                          'name': '2019',
                          'recordCount': '2406726',
                          'sizeKey': '17721226'},
        ...
        try:
            logger.info(f"Recreating table: {self.rsf_array_table_name}")
            response = self.glue.delete_table(
                DatabaseName=self.database_name,
                Name=self.rsf_array_table_name
            )
        except ClientError as e:
            if e.response['Error']['Code'] == 'EntityNotFoundException':
                logger.debug(r"Skipping table deletion as it does not exist: {self.rsf_array_table_name}")
        ...
        response = self.glue.create_table(
            ...
            TableInput={
                ...
                'PartitionKeys': [{'Name': 'city_year', 'Type': 'string'}],
                'Retention': 0,
                'Parameters': {'averageRecordSize': '11',
                              'classification': 'parquet',
                              'compressionType': 'none',
                              'objectCount': '830',
                              'recordCount': '230652018',
                              'sizeKey': '1574548587',
                              'typeOfData': 'file'},
                'StorageDescriptor': {'BucketColumns': [],
                                 'Columns': [{'Name': 'measured_on',
                                 ...
                                 'Compressed': False,
                                 'InputFormat': 'org.apache.hadoop.hive.ql.io.parquet.MapredParquetInputFormat',
                                 'Location': f'{self.rsf_array_bucket}/',
                                 'NumberOfBuckets': -1,
                                 'OutputFormat': 'org.apache.hadoop.hive.ql.io.parquet.MapredParquetOutputFormat',
                                 'Parameters': {'averageRecordSize': '536',
                                                'classification': 'parquet',
    ...
    def create_table(self, table_input, partitions_array, table_name, partitions_template, source_bucket):
        try:
        ...
        for partition in partitions_array:
            # pprint(partition)
            count = count + 1
            thisTemplate = partitions_template
            thisTemplate['StorageDescriptor']['Location'] = f"{source_bucket}/{partition['name']}/"
            thisTemplate['StorageDescriptor']['Parameters']['averageRecordSize'] = partition['averageRecordSize']
            thisTemplate['StorageDescriptor']['Parameters']['recordCount'] = partition['recordCount']
            thisTemplate['StorageDescriptor']['Parameters']['sizeKey'] = partition['sizeKey']
            thisTemplate['Values'][0] = partition['name']
    ...
    def create_rsf_array_table(self):
        partitions_array = [{'averageRecordSize': '17',
                            'name': '2019',
                            'recordCount': '2406726',
                            'sizeKey': '17721226'},
                           {'averageRecordSize': '14',
        ...
        partition_template = self.get_rsf_array_template()

        source_bucket = self.rsf_array_bucket

        table_name = self.rsf_array_table_name

        self.create_table(table_input, partitions_array, table_name, partition_template, source_bucket)
